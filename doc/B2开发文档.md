...
加装了无线网卡用于连接WiFi，在连接**清华4F**的一个情况下IP地址为`172.16.20.236`。  
个人PC或websocket服务端（以下称为`PC3`）在连接**清华4F**的一个情况下IP地址为`172.16.20.206`。  
`PC2`的有线网卡设备id为`eth0`，无线网卡设备id为`wlxfc221c5074c3`。  
...
&emsp;&emsp;│  
&emsp;&ensp;&nbsp;▼  
PC2无线接口 (wlxfc221c5074c3, 172.16.20.236)  
&emsp;&emsp;│  NAT转发  
&emsp;&ensp;&nbsp;▼    
...
sudo iptables -t nat -A PREROUTING -i wlxfc221c5074c3 -p tcp --dport 8551 -j DNAT --to-destination 192.168.123.161:8551
...
sudo iptables -t nat -A POSTROUTING -o wlxfc221c5074c3 -s 192.168.123.161 -p tcp --sport 8551 -j SNAT --to-source 172.16.20.236
```
...
sudo iptables -A FORWARD -i wlxfc221c5074c3 -o eth0 -d 192.168.123.161 -p tcp --dport 8551 -j ACCEPT
# 允许转发从有线到无线、来自PC1端口8551的返回包
sudo iptables -A FORWARD -i eth0 -o wlxfc221c5074c3 -s 192.168.123.161 -p tcp --sport 8551 -j ACCEPT
```
8551为前相机使用的端口。对后相机，将8551改为8552再设置一遍即可。  
## 5.保存配置
```bash
...
```bash
# 监听无线网卡
sudo tcpdump -i wlxfc221c5074c3 host 172.16.20.206 and port 8551
# 监听有线网卡
